...
antoine_path = os.path.join("Antoine", "Antoine.xlsx")

# Load Antoine Data
if not os.path.exists(antoine_path):
    logger.error("Antoine.xlsx not found in 'Antoine' folder!")
    exit()

antoine_df = pd.read_excel(antoine_path)

# Ensure the necessary columns exist
if not {"SMILES", "A", "B", "C"}.issubset(antoine_df.columns):
    logger.error("Antoine.xlsx must contain 'SMILES', 'A', 'B', 'C' columns!")
    exit()
...
def get_antoine_params(smiles):
    match = antoine_df[antoine_df["SMILES"] == smiles]
    if match.empty:
        logger.warning(f"SMILES '{smiles}' not found in Antoine.xlsx. Using default values.")
        return None, None, None
    return match.iloc[0]["A"], match.iloc[0]["B"], match.iloc[0]["C"]
...
def plot_vle_manual(smiles_i, smiles_j):
    name_i, name_j = get_compound_name(smiles_i), get_compound_name(smiles_j)

    # Get Antoine coefficients
    A_i, B_i, C_i = get_antoine_params(smiles_i)
    A_j, B_j, C_j = get_antoine_params(smiles_j)
    
    if None in [A_i, B_i, C_i, A_j, B_j, C_j]:
        logger.error("Missing Antoine coefficients. Check Antoine.xlsx!")
        return
    ...
    # Major grid lines (thicker and solid)
    plt.grid(which='major', linestyle='-', linewidth=0.6, color='gray')

    # Minor grid lines (thinner and dashed)
    plt.grid(which='minor', linestyle='--', linewidth=0.4, color='gray')
    plt.show()

# Run the VLE plot with Antoine.xlsx integration
plot_vle_manual("O", "CCO")