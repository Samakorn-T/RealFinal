...
y = data["Bij"]  # Modify if training for other targets
...
for threshold in thresholds:
    ...
    selected_features_txt = os.path.join(xgb_folder, f"selected_features_{target_name}_thresh{threshold}.txt")
    selected_features_excel = os.path.join(xgb_folder, f"selected_features_{target_name}_thresh{threshold}.xlsx")
    ...
    # Save the scaler
    scaler_path = os.path.join(xgb_folder, f"scaler_{target_name}_thresh{threshold}.pkl")
    ...
    logger.info(f"Starting Bayesian Optimization for XGBoost on target '{target_name}' with threshold {threshold}...")
    ...
    model_path = os.path.join(xgb_folder, f"best_xgboost_model_{target_name}_thresh{threshold}.pkl")
    joblib.dump(best_xgb, model_path)
    logger.info(f"Best XGBoost model for target '{target_name}' with threshold {threshold} saved to {model_path}")
    
    # **Step 10: Save Hyperparameter Log**
    log_file = os.path.join(xgb_folder, f"tuning_log_{target_name}_thresh{threshold}.txt")
    with open(log_file, "w") as f:
        f.write(f"Target: {target_name}\n")
        f.write(f"Threshold: {threshold}\n")
        f.write(f"Best Parameters:\n{bayes_search.best_params_}\n")
    logger.info(f"Hyperparameter log for target '{target_name}' with threshold {threshold} saved to {log_file}")
    ...
    logger.info(f"Cross-validation R² scores for target '{target_name}' with threshold {threshold}: {cv_results}")
