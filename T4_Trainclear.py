...
# **Step 3: Define Files for Training**
data_files = ["Expanded_Fingerprints_Data.csv", "Expanded_Fingerprints_Data_Cleaned.csv"]
data_folder = "Prepare_datavisual"
threshold = 0.03  # Fixed threshold

for data_file in data_files:
    dataset_name = data_file.replace(".csv", "")  # Extract dataset name for saving
    ...
    logger.info(f"Training model with VarianceThreshold = {threshold} on {data_file}")
    ...
    selected_features_txt = os.path.join(xgb_folder, f"selected_features_{dataset_name}.txt")
    selected_features_excel = os.path.join(xgb_folder, f"selected_features_{dataset_name}.xlsx")
    ...
    logger.info(f"Starting Bayesian Optimization for XGBoost on {dataset_name}...")
    ...
    model_path = os.path.join(xgb_folder, f"best_xgboost_model_{dataset_name}.pkl")
    joblib.dump(best_xgb, model_path)
    logger.info(f"Best XGBoost model for {dataset_name} saved to {model_path}")
    
    # **Step 10: Save Hyperparameter Log**
    log_file = os.path.join(xgb_folder, f"tuning_log_{dataset_name}.txt")
    with open(log_file, "w") as f:
        f.write(f"Dataset: {dataset_name}\n")
        f.write(f"Threshold: {threshold}\n")
        f.write(f"Best Parameters:\n{bayes_search.best_params_}\n")
    logger.info(f"Hyperparameter log for {dataset_name} saved to {log_file}")
    ...
    logger.info(f"Cross-validation R² scores for {dataset_name}: {cv_results}")
